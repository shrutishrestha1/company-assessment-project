      ...
      <div className="card" style={{ padding: 0 }}>
        <div className="table-container">
          {loading ? (
            ...
            <div className="empty-state"><BarChart3 className="empty-icon-svg" strokeWidth={1.25} aria-hidden /><p>No transactions found</p><small>Try adjusting your filters</small></div>
          ) : (
            <table className="data-table">
              ...
              <tbody>
                {transactions.map((t) => (
                  <tr key={t.id}>
                    <td><span className="mono" style={{ fontSize: 12, color: 'var(--accent)' }}>{t.reference_no}</span></td>
                    <td>
                    ...
                    <td>
                      ...
                      <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>Nepal · {t.receiver_phone}</div>
                    </td>
        ...
        >
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
              <span className="mono" style={{ color: 'var(--accent)', fontSize: 14 }}>{detail.reference_no}</span>
              <span className={`badge ${STATUS_COLORS[detail.status] || 'badge-neutral'}`}>{detail.status}</span>
            </div>

            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 20 }}>
              {[
                { label: 'Sender (Japan)', value: detail.sender_name, sub: detail.sender_email || detail.sender_phone },
                { label: 'Receiver (Nepal)', value: detail.receiver_name, sub: detail.receiver_phone },
              ].map((item) => (
            ...
            {[
              ...
              <div key={row.label} style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 0', borderBottom: '1px solid var(--border-light)', fontSize: 14 }}>
                ...
                <span className="mono" style={{ fontWeight: row.bold ? 700 : 500, color: row.color, fontSize: row.bold ? 16 : 14 }}>{row.value}</span>
              </div>
            ))}
            ...
            {detail.bank_name && (
              <div style={{ marginTop: 12, padding: '10px 14px', background: 'var(--bg-secondary)', borderRadius: 8, fontSize: 13 }}>
                ...
                {detail.bank_account && <span className="mono" style={{ color: 'var(--text-secondary)', marginLeft: 8 }}>{detail.bank_account}</span>}
              </div>
            )}

            {detail.remarks && (
              <div style={{ marginTop: 10, padding: '10px 14px', background: 'var(--bg-secondary)', borderRadius: 8, fontSize: 13, color: 'var(--text-secondary)', display: 'flex', alignItems: 'flex-start', gap: 8 }}>
                <MessageSquare size={16} strokeWidth={2} style={{ flexShrink: 0, marginTop: 2, color: 'var(--text-muted)' }} aria-hidden />
                <span><span style={{ color: 'var(--text-muted)', fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.04em' }}>Remarks</span><br />{detail.remarks}</span>
              </div>
            )}
